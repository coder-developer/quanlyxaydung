...
import OpsSimulator from './components/OpsSimulator';
...
import { fetchCollection, batchSaveCollection, testConnection } from './lib/firestoreSync';
...
export default function App() {
  const [activeTab, setActiveTab] = useState<'dashboard' | 'schema' | 'flows' | 'sim' | 'tech' | 'liabilities' | 'hr' | 'warehouse' | 'equipment' | 'journal' | 'company' | 'drive'>('dashboard');
  ...
  const handleLoginSuccess = useCallback((role: UserRole, fullName?: string) => {
  ...
  const isTabRestricted = useCallback((tab: string) => {
    if (currentUserRole === 'SiteManager') {
      const siteManagerRestricted = ['schema', 'liabilities', 'hr', 'company', 'journal', 'tech'];
  ...
  const [approvals, setApprovals] = useState<ApprovalRequest[]>(() => {
    const saved = localStorage.getItem('erp_approvals');
  ...
  const [laborContracts, setLaborContracts] = useState<LaborContract[]>(() => {
    const saved = localStorage.getItem('erp_labor_contracts');
    return saved ? JSON.parse(saved) : initialLaborContracts;
  });

  const [constructionTasks, setConstructionTasks] = useState<ConstructionTask[]>(() => {
    const saved = localStorage.getItem('erp_construction_tasks');
    return saved ? JSON.parse(saved) : initialConstructionTasks;
  });
  ...
  const securedSetTimesheets = useMemo(() => secureSetState(setTimesheets, ['CEO', 'Accountant'], 'Bảng Chấm công'), [secureSetState, setTimesheets]);
    ...
    <div className="flex flex-col md:flex-row h-screen w-full bg-slate-50 font-sans text-slate-900 overflow-hidden" id="app-root">
      ...
      <aside className="w-full md:w-64 bg-slate-900 flex flex-col shrink-0 border-b md:border-b-0 md:border-r border-slate-850">
        ...
        <div className="p-4 md:p-6 flex items-center justify-between md:justify-start gap-3 border-b border-slate-800 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-[8px] text-slate-500 font-bold uppercase tracking-wider mt-1">PROTOTYPE V1.0</span>
            </div>
          </div>
        ...
        <nav className="flex flex-row md:flex-col gap-1.5 md:space-y-1.5 p-2 md:p-4 overflow-x-auto md:overflow-y-auto scrollbar-none md:flex-1 shrink-0">
          ...
          <button
            onClick={() => handleTabClick('tech', 'Đề xuất Công nghệ & CTO')}
            className={`flex-none md:w-full flex items-center justify-between px-3.5 py-2 rounded-md text-xs font-semibold whitespace-nowrap transition-colors text-left ${
              activeTab === 'tech' 
                ? 'bg-blue-600 text-white shadow-xs' 
                : isTabRestricted('tech')
                  ? 'text-slate-600 bg-slate-950/20 cursor-not-allowed opacity-55'
                  : 'text-slate-400 hover:text-white hover:bg-slate-800'
            }`}
          >
            <div className="flex items-center gap-2.5 md:gap-3">
              ...
              <span>Đề xuất Công nghệ & CTO</span>
            </div>
            {isTabRestricted('tech') && <Lock className="w-3.5 h-3.5 text-slate-500 shrink-0" />}
          </button>
        ...
        <header className="h-auto py-3 md:h-16 bg-white border-b border-slate-200 flex flex-col md:flex-row items-start md:items-center justify-between px-4 md:px-8 gap-3 shrink-0 shadow-3xs">
          <div className="shrink-0">
            <h1 className="text-xs md:text-sm lg:text-base font-black text-slate-800 tracking-tight flex items-center gap-1.5 md:gap-2">
              ...
              {activeTab === 'tech' && '💻 Đề Xuất Công Nghệ & Tư Vấn CTO'}
          ...
          <div className="flex items-center gap-3 w-full sm:w-auto justify-between sm:justify-end shrink-0">
            ...
            {/* Quick Status Pill */}
            <div className="hidden sm:flex items-center gap-1.5 bg-emerald-50 px-2.5 py-1.5 rounded border border-emerald-100 shrink-0 shadow-3xs">
              <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></div>
              <span className="text-[9px] font-extrabold text-emerald-700 uppercase tracking-wider">SYSTEM LIVE</span>
            </div>
          ...
          <div className="max-w-7xl mx-auto space-y-6">
            ...
            <footer className="pt-8 border-t border-slate-200 text-slate-400 text-xs flex flex-col sm:flex-row items-center justify-between gap-4 mt-12 pb-6">
              <div className="flex items-center gap-2">
                ...
                <span className="font-semibold text-slate-500">Thiết kế bởi Giám Đốc Công Nghệ (CTO) & Kiến Trúc Sư Hệ Thống</span>
              </div>
