...
export default function MasterDataEditor(props: Props) {
  ...
  const rows = () => {
    if (tab === 'projects') return props.projects.map(item => <Row key={item.id} id={item.id} code={codeField(`project:${item.id}`, item.code || item.id, 'DA', props.projects.map(value => value.code || value.id), code => props.setProjects(values => values.map(value => value.id === item.id ? { ...value, code } : value)))} name={nameField(item.name, name => props.setProjects(values => values.map(value => value.id === item.id ? { ...value, name } : value)), 'Tên dự án')}/>);
    if (tab === 'employees') return props.employees.map(item => <Row key={item.id} id={item.id} code={codeField(`employee:${item.id}`, item.code || item.id, 'NV', props.employees.map(value => value.code || value.id), code => props.setEmployees(values => values.map(value => value.id === item.id ? { ...value, code } : value)))} name={nameField(item.name, name => props.setEmployees(values => values.map(value => value.id === item.id ? { ...value, name } : value)), 'Họ tên')}/>);
    if (tab === 'partners') return props.contractors.map(item => <Row key={item.id} id={item.id} code={codeField(`partner:${item.id}`, item.code || item.id, 'DT', props.contractors.map(value => value.code || value.id), code => props.setContractors(values => values.map(value => value.id === item.id ? { ...value, code } : value)))} name={nameField(item.name, name => props.setContractors(values => values.map(value => value.id === item.id ? { ...value, name } : value)), 'Tên đối tác')}/>);
    if (tab === 'contracts') return props.contracts.map(item => <Row key={item.id} id={item.id} code={codeField(`contract:${item.id}`, item.contractNumber, 'HD', props.contracts.map(value => value.contractNumber), contractNumber => props.setContracts(values => values.map(value => value.id === item.id ? { ...value, contractNumber } : value)))} name={nameField(item.title, title => props.setContracts(values => values.map(value => value.id === item.id ? { ...value, title } : value)), 'Tên hợp đồng')}/>);
    if (tab === 'inventory') return props.inventoryItems.map(item => <Row key={item.id} id={item.id} code={codeField(`inventory:${item.id}`, item.code || item.id, 'VT', props.inventoryItems.map(value => value.code || value.id), code => props.setInventoryItems(values => values.map(value => value.id === item.id ? { ...value, code } : value)))} name={nameField(item.name, name => props.setInventoryItems(values => values.map(value => value.id === item.id ? { ...value, name } : value)), 'Tên vật tư')}/>);
    return props.equipment.map(item => <Row key={item.id} id={item.id} code={codeField(`equipment:${item.id}`, item.code || item.id, 'TB', props.equipment.map(value => value.code || value.id), code => props.setEquipment(values => values.map(value => value.id === item.id ? { ...value, code } : value)))} name={nameField(item.name, name => props.setEquipment(values => values.map(value => value.id === item.id ? { ...value, name } : value)), 'Tên thiết bị')}/>);
  };
  ...
  return <div className="space-y-4">
    ...
    <div className="overflow-hidden rounded-2xl border bg-white">
      <div className="grid grid-cols-[150px_1fr_150px] gap-3 bg-slate-100 px-4 py-3 text-xs font-black uppercase text-slate-500"><div>Mã = ID nội bộ</div><div>Tên / Tiêu đề</div><div>ID hiện tại</div></div>
      {rows()}
    </div>
  </div>;
}

function Row({ id, code, name }: { key?: string; id: string; code: ReactNode; name: ReactNode }) {
  return <div className="grid grid-cols-[150px_1fr_150px] gap-3 border-t p-3">{code}{name}<code className="self-center truncate text-xs font-bold text-blue-700" title={id}>{id}</code></div>;
}
