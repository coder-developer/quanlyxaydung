...
export default function MasterDataEditor(props: Props) {
  ...
  const rows = () => {
    ...
    if (tab === 'partners') return props.contractors.map(item => <Row key={item.id} id={item.id} onDelete={() => deleteItem(item.id)} code={codeField(`partner:${item.id}`, item.code || item.id, 'DT', props.contractors.map(value => value.code || value.id), code => props.setContractors(values => values.map(value => value.id === item.id ? { ...value, code } : value)))} name={<div className="grid gap-2 lg:grid-cols-3">{nameField(item.name, name => props.setContractors(values => values.map(value => value.id === item.id ? { ...value, name } : value)), 'Tên đối tác')}{nameField(item.taxCode || '', taxCode => props.setContractors(values => values.map(value => value.id === item.id ? { ...value, taxCode } : value)), 'Mã số thuế')}{nameField(item.officeAddress || '', officeAddress => props.setContractors(values => values.map(value => value.id === item.id ? { ...value, officeAddress } : value)), 'Địa chỉ văn phòng')}</div>}/>);
